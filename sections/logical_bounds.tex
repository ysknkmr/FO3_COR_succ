...
\begin{remark}[Bounds without anchors]\label{rem:anchor-free}
The anchors are needed because $\Psi_k$ and $\Phi_k$ are witness queries
(Lemma~\ref{lem:where}). If the body may also read the pair $(x,y)$, the
bounds of Theorems~\ref{main:omega} and~\ref{main:phi}, and those of
Section~\ref{sec:matrix}, follow on $\M$ without anchors; see
Appendix~\ref{app:anchor-free}.
\end{remark}
...
advance the question is a different one. Addressing the $2k$ symbols by
paths of length $O(\log k)$ in a single binary relation gives a gap of
$2^{\Omega(n/\log n)}$ (Theorem~\ref{thm:af-fixed}), with positive
formulas, at the price of quantifier depth $O(\log k)$. We do not know whether a gap of
$2^{\Omega(n)}$ holds over a fixed vocabulary.
\end{remark}
